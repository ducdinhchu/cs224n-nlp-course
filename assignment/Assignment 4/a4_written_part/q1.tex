...
\begin{parts}
    ...
    \ifans {
        The function masks the paddings in all sentences to boolean value 1 and other tokens to 0. They are then converted to $-\infty$ in attention score $\be_t$.
        In the calculation of $\alpha_t$, they become zero since $\exp(-\infty) = 0$.
        Therefore, they have zero effect in the attention output $\ba_t$ (weighted average).
        This is necessary since $\texttt{<pad>}$ tokens are not part of the original source data and do not contain meaningful information and should not be attended to or affect the attention output.
    } \fi \newline
\end{parts}
...
(Windows) run.bat test
    ...
    \ifans{
        Corpus BLEU: 13.110528453394458 (with default hyperparameters)
    } \fi \newline
    ...
    \ifans{
        \begin{enumerate}[i.]
            \item \\
            \textbf{Advantage}: The \textit{dot product attention} does not require additional model parameters and is easier to compute. \\
            \textbf{Disadvantage}: Vectors $\bs_t$ (the \textit{query}) and $\bh_i$ (the \textit{key}) must be of the same dimension.
            The \textit{multiplicative attention}, however, has a projection matrix $\bW$.
            \item
            \textbf{Advantage}: The \textit{additive attention} introduces a non-linearity ($\tanh$) and separate weight matrices for two input vectors.
            Therefore, it is more likely to have a more complex functionality and may have a better performance.\\
            \textbf{Disadvantage}: It is more computationally expensive to train.
        \end{enumerate}
    } \fi \newline
\end{parts}
