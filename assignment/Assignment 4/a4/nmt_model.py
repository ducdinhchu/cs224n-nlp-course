...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Four linear layers with NO bias.
        ...
        self.target_vocab_projection = nn.Linear(self.hidden_size, len(self.vocab.tgt),
                                                 bias=False)  # Size of target vocabulary.
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        @return enc_hiddens (Tensor): Tensor of hidden units with shape (b, src_len, h*2), where
                                        b = batch size, src_len = maximum source sentence length, h = hidden size.
        @return dec_init_state (tuple(Tensor, Tensor)): Tuple of tensors representing the decoder's initial
        ...
        X = pack_padded_sequence(X, lengths=source_lengths, enforce_sorted=True)  # Sequences already sorted by length.
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        # Recover the padding (reverse operation of pack_padded_sequence()) and switch dim 0 and 1.
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)  # NOTE: This guy also returns a list of lengths.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.target(target_padded)  # Look up target embeddings.
        for Y_t in torch.split(Y, split_size_or_sections=1):  # Break the ground truth into single tensors.
            Y_t = torch.squeeze(Y_t, dim=0)  # Without `dim=0`: all the dimensions of size 1 will be removed.
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)  # Concatenate over dimension 1.
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)  # Append the new output to the list.
            o_prev = o_t  # Update the combined-output vector of the previous step.
        combined_outputs = torch.stack(combined_outputs, dim=0)  # Make the list a tensor.
    ...
    def generate_sent_masks(self, enc_hiddens: torch.Tensor, source_lengths: List[int]) -> torch.Tensor:
        ...
        for e_id, src_len in enumerate(source_lengths):
            enc_masks[e_id, src_len:] = 1  # Masks start from end of sequence.
